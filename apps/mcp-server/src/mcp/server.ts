  ...
  // ── Card curation ─────────────────────────────────────────────────────────
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        action: {
          type: "string",
          enum: ["shortlist", "skip", "clear"],
          description: "shortlist = queue for drafting; skip = skip this cycle; clear = remove status.",
        },
      },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        memoryType: {
          ...
          description: "voice_examples, style_rules, audience_insights, do_not_say, successful_posts, campaign_context, source_preferences, visual_style (one-time image style setup), voice_profile (one-time audio voice setup), face_profile (one-time face/appearance setup for image/video likeness)",
        },
      },
      required: ["entries"],
    },
