...
export async function handleTool(raw: unknown, ctx: ToolContext) {
  ...
  switch (parsed.action) {
    ...
    case "list": {
      ...
      return {
        content: [{ type: "text" as const, text: drafts.length ? `${drafts.length} draft(s) saved.${drafts.slice(0, 5).map(d => `\n  [${d.platform ?? "?"}] ${(d.content ?? "").slice(0, 60)}${d.content.length > 60 ? "…" : ""}`).join("")}${drafts.length > 5 ? `\n  … and ${drafts.length - 5} more` : ""}` : "No saved drafts for this workspace yet." }],
