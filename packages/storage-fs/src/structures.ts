...
export function saveHarvestOutput(rawCards: CardInput[], _seenUrls?: Set<string>, workspaceId?: string): string {
  ...
  // Merge with existing harvest if one is already saved today.
  let existingCards: StructureCard[] = [];
  let existingCurationState: HarvestBundle["curationState"] = {};
  let outputDir: string;
  if (
    fs.existsSync(paths.latestHarvestPointer)
  ) {
    const existingBundlePath = fs.readFileSync(paths.latestHarvestPointer, "utf-8").trim();
    if (existingBundlePath && fs.existsSync(existingBundlePath)) {
      try {
        const existing = HarvestBundleSchema.parse(JSON.parse(fs.readFileSync(existingBundlePath, "utf-8")));
        existingCards = existing.cards;
        existingCurationState = existing.curationState ?? {};
  ...
  const newCards: StructureCard[] = rawCards.map((raw, index) => ({
