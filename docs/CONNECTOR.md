...
```bash
docker exec -it quillby node /app/dist/cli/keys.js create-user you@example.com yourpassword "Your Name"
...
```bash
docker exec -it quillby node /app/dist/cli/keys.js list <userId>
docker exec -it quillby node /app/dist/cli/keys.js revoke <keyId>
```

### Local monorepo / non-Docker
...
```bash
# Option A — connect to the container directly:
fly ssh console
node /app/dist/cli/keys.js create-user you@example.com yourpassword "Your Name"
node /app/dist/cli/keys.js create <userId> quillby-connector

# Option B — run keys commands locally against the remote Turso DB:
QUILLBY_AUTH_DB_URL=libsql://quillby-<org>.turso.io \
LIBSQL_AUTH_TOKEN=<token> \
pnpm --filter @vncsleal/quillby keys create-user you@example.com yourpassword "Your Name"
